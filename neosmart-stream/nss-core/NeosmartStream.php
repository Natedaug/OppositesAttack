 ...
 ****************************************************************************/
	...
	private $el = array(
		...
		'sp' 	=> "<a title='neosmart STREAM - Facebook Newsfeed &amp; Twitter Plugin' target='_blank' href='https://neosmart-stream.de/' style='text-decoration:none;color:#888888!important;'><span style='color:#ff7800!important;background:url(nss-root/nss-core/nss-icon.png) no-repeat 0 center!important;display:inline-block!important;padding:2px 2px 4px 22px!important'>neosmart STREAM</span> - Social Plugin</a>",
 ...
 **************************************************************************/
	...
	public function init() {
		if($this->get('date_default_timezone')!='') date_default_timezone_set($this->get('date_default_timezone'));
		//setlocale (LC_TIME, '');
		//setlocale (LC_TIME, $this->get('locale_time'));
	}
 ...
 **************************************************************************/
	...
	public function getChannelField($id_type,$value){
		$cut = strrpos($id_type,'|');
		$id = trim(substr($id_type,0,$cut));
		$type = trim(substr($id_type,$cut+1));

		foreach($this->channel_list as $c){
			 if($c['id']==$id && $c['type']==$type) return $c[$value];
		}
 ...
 **************************************************************************/
 	...
 	public function getBaseURL(){
		...
		foreach($base_urls as $u){
			...
			//Wenn aktuelle URL Port enthält, dann anhängen
			if(strpos($host,':')) $base_host .= ':'.parse_url($u,PHP_URL_PORT);
			//Übereinstimmung ausgeben
		...
		if($base===false){
			...
			if($current_nss_folder){
				...
				$this->saveBaseURL(trim(implode(',',$base_urls),","));
 ...
 **************************************************************************/
	...
	function saveBaseURL($urls){
		...
		if($fh){
			...
			$this->redirectTo('?error=file_permissions');
		}
	}
 ...
 **************************************************************************/
	...
	function getLastUpdate(){
		...
		if(!$ft) return "Never";
		return utf8_encode(strftime($this->get('date_time_format'), $ft));
	}
 ...
 **************************************************************************/
	...
	function readChannelProfile($channel){
		...
		if($error===false){
			...
			$data .= "\n\t<screenname>".$this->cdata($screenname)."</screenname>";
			$data .= "\n\t<link>".$this->cdata($link)."</link>";
			$data .= "\n\t<avatar>".$this->cdata($avatar)."</avatar>";
 ...
 * @update: 1.6.2 (Überprüfung ob id UND type stimmen)
 **************************************************************************/
 	
	private function isPartOfGroup($channelArray){
		...
		foreach($this->group_list as $group){
			if($group['id']==$current){
				...
				foreach(explode(',',$str) as $channel){
					if($this->getChannelField($channel,'id')==$channelArray['id'] && $this->getChannelField($channel,'type')==$channelArray['type'])
						return true;	
				}
			}
		}
 ...
 **************************************************************************/
	...
	function mergeChannels($theme,$group=false){
		...
		for($k=0;$k<count($this->channel_list);$k++){
			...
			if(!$this->isPartOfGroup($this->channel_list[$k])) continue;
	...
	function sortData($data){
		...
		if(md5($el)!='4895adc21882e9a59e4c4b3983d64a96'){
 ...
 **************************************************************************/
	...
	function insertDataIntoTemplate($item){
		...
		for($position=0;$position<count($item);$position++){
			...
			$extras_facebook_source = !$is_facebook || is_array($nss['extras']['facebook']['source']) ? '' : $this->preventAutoplay($this->getSafeURL($nss['extras']['facebook']['source']));
			...
			//Facebook Images
			//Wenn das Origional kleiner als die gewünschte Größe ist, wird die Originalgröße angezeigt
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_2048'])){
				$extras_facebook_image_2048 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_2048 = $nss['extras']['facebook']['images']['image_2048']['source'];
				$extras_facebook_image_2048_width = $nss['extras']['facebook']['images']['image_2048']['width'];
				$extras_facebook_image_2048_height = $nss['extras']['facebook']['images']['image_2048']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_960'])){
				$extras_facebook_image_960 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_960 = $nss['extras']['facebook']['images']['image_960']['source'];
				$extras_facebook_image_960_width = $nss['extras']['facebook']['images']['image_960']['width'];
				$extras_facebook_image_960_height = $nss['extras']['facebook']['images']['image_960']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_720'])){
				$extras_facebook_image_720 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_720 = $nss['extras']['facebook']['images']['image_720']['source'];
				$extras_facebook_image_720_width = $nss['extras']['facebook']['images']['image_720']['width'];
				$extras_facebook_image_720_height = $nss['extras']['facebook']['images']['image_720']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_600'])){
				$extras_facebook_image_600 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_600 = $nss['extras']['facebook']['images']['image_600']['source'];
				$extras_facebook_image_600_width = $nss['extras']['facebook']['images']['image_600']['width'];
				$extras_facebook_image_600_height = $nss['extras']['facebook']['images']['image_600']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_480'])){
				$extras_facebook_image_480 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_480 = $nss['extras']['facebook']['images']['image_480']['source'];
				$extras_facebook_image_480_width = $nss['extras']['facebook']['images']['image_480']['width'];
				$extras_facebook_image_480_height = $nss['extras']['facebook']['images']['image_480']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_320'])){
				$extras_facebook_image_320 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_320 = $nss['extras']['facebook']['images']['image_320']['source'];
				$extras_facebook_image_320_width = $nss['extras']['facebook']['images']['image_320']['width'];
				$extras_facebook_image_320_height = $nss['extras']['facebook']['images']['image_320']['height'];
			}
			if(!$is_facebook || empty($nss['extras']['facebook']['images']['image_130'])){
				$extras_facebook_image_130 = $extras_facebook_picture;
			} else{
				$extras_facebook_image_130 = $nss['extras']['facebook']['images']['image_130']['source'];
				$extras_facebook_image_130_width = $nss['extras']['facebook']['images']['image_130']['width'];
				$extras_facebook_image_130_height = $nss['extras']['facebook']['images']['image_130']['height'];
			}
			...
			//Hashtags
			if($is_facebook){
				$content = $this->autoLinkFacebookHashTags($content);
				$extras_facebook_message = $this->autoLinkFacebookHashTags($extras_facebook_message);
				$extras_facebook_event_description = $this->autoLinkFacebookHashTags($extras_facebook_message);
			}
			elseif($is_twitter){
				$content = $this->autoLinkTwitterHashTags($content);
			}
			...
			if($extras_facebook_count_comments>1){
				...
				for($d=0;$d<$extras_facebook_count_comments;$d++){
					...
					$extras_facebook_comments_content = $this->autoLinkFacebookHashTags($this->autoLink($tmp['comment'][$d]['content']));
				...
				$extras_facebook_comments_content = $this->autoLinkFacebookHashTags($this->autoLink($tmp['comment']['content']));
...
/**************************************************************************
 * Facebook Data in NSS Format umwandeln
 * @since: 1.0
 * @update: 1.6.3
 **************************************************************************/
	...
	function convertFacebookChannel($id,$fbdata,$access_token,$limit){ 
		...
		for($k=0;$k< count($posts);$k++){
			...
			if($type=='video'){
				$video_src = $p->source;
				//Facebook Videos
				if(strpos($video_src,'akamaihd.net/')!==false){
					$video_src = "https://www.facebook.com/video/embed?video_id=".$object_id;
				}
				//Instagramm Videos > auf Typ Photo umstellen
				if(strpos($p->link,'://instagram.com/')!==false){
					$type = 'photo';
				}
			}
			...
			$output .= "\n\t<updated>"; if(isset($p->updated_time)) $output .= $this->transformDate($p->updated_time,'c'); $output .='</updated>';
			$output .= "\n\t<content></content>";
			$output .= "\n\t<author>";
			...
			$output .= "\n\t<extras>";
				$output .= "\n\t\t<facebook>";
					...
					$output .= "\n\t\t\t<images>";
						if(isset($object_json->images) && count($object_json->images)>0):
							$original 	= $object_json->images[0];
							$image_2048	= array($original->source,$original->width,$original->height);
							$image_960	= array($original->source,$original->width,$original->height);
							$image_720	= array($original->source,$original->width,$original->height);
							$image_600	= array($original->source,$original->width,$original->height);
							$image_480	= array($original->source,$original->width,$original->height);
							$image_320	= array($original->source,$original->width,$original->height);
							$image_130	= array($original->source,$original->width,$original->height);
								
							foreach($object_json->images as $image){								
								//Portrait
								if($object_json->height > $object_json->width){ 
									if($image->height>959&&$image->height<$image_960[2]) $image_960=array($image->source,$image->width,$image->height);
									if($image->height>719&&$image->height<$image_720[2]) $image_720=array($image->source,$image->width,$image->height);
									if($image->height>599&&$image->height<$image_600[2]) $image_600=array($image->source,$image->width,$image->height);
									if($image->height>479&&$image->height<$image_480[2]) $image_480=array($image->source,$image->width,$image->height);
									if($image->height>319&&$image->height<$image_320[2]) $image_320=array($image->source,$image->width,$image->height);
									if($image->height>129&&$image->height<$image_130[2]) $image_130=array($image->source,$image->width,$image->height);
								}
								//Landscape
								else{
									if($image->width>959&&$image->width<$image_960[1]) 	$image_960=array($image->source,$image->width,$image->height);
									if($image->width>719&&$image->width<$image_720[1]) 	$image_720=array($image->source,$image->width,$image->height);
									if($image->width>599&&$image->width<$image_600[1])	$image_600=array($image->source,$image->width,$image->height);
									if($image->width>479&&$image->width<$image_480[1]) 	$image_480=array($image->source,$image->width,$image->height);
									if($image->width>319&&$image->width<$image_320[1]) 	$image_320=array($image->source,$image->width,$image->height);
									if($image->width>129&&$image->width<$image_130[1]) 	$image_130=array($image->source,$image->width,$image->height);
								}
							}
							$output .= "\n\t\t\t\t<image_2048>\n\t\t\t\t\t<source>".$this->cdata($image_2048[0])."</source>\n\t\t\t\t\t<width>".$image_2048[1]."</width>\n\t\t\t\t\t<height>".$image_2048[2]."</height>\n\t\t\t\t</image_2048>";
							$output .= "\n\t\t\t\t<image_960>\n\t\t\t\t\t<source>".$this->cdata($image_960[0])."</source>\n\t\t\t\t\t<width>".$image_960[1]."</width>\n\t\t\t\t\t<height>".$image_960[2]."</height>\n\t\t\t\t</image_960>";
							$output .= "\n\t\t\t\t<image_720>\n\t\t\t\t\t<source>".$this->cdata($image_720[0])."</source>\n\t\t\t\t\t<width>".$image_720[1]."</width>\n\t\t\t\t\t<height>".$image_720[2]."</height>\n\t\t\t\t</image_720>";
							$output .= "\n\t\t\t\t<image_600>\n\t\t\t\t\t<source>".$this->cdata($image_600[0])."</source>\n\t\t\t\t\t<width>".$image_600[1]."</width>\n\t\t\t\t\t<height>".$image_600[2]."</height>\n\t\t\t\t</image_600>";
							$output .= "\n\t\t\t\t<image_480>\n\t\t\t\t\t<source>".$this->cdata($image_480[0])."</source>\n\t\t\t\t\t<width>".$image_480[1]."</width>\n\t\t\t\t\t<height>".$image_480[2]."</height>\n\t\t\t\t</image_480>";
							$output .= "\n\t\t\t\t<image_320>\n\t\t\t\t\t<source>".$this->cdata($image_320[0])."</source>\n\t\t\t\t\t<width>".$image_320[1]."</width>\n\t\t\t\t\t<height>".$image_320[2]."</height>\n\t\t\t\t</image_320>";
							$output .= "\n\t\t\t\t<image_130>\n\t\t\t\t\t<source>".$this->cdata($image_130[0])."</source>\n\t\t\t\t\t<width>".$image_130[1]."</width>\n\t\t\t\t\t<height>".$image_130[2]."</height>\n\t\t\t\t</image_130>";
						endif;
 ...
 **************************************************************************/
 	...
 	function readTwitterChannel($channel){
		...
		$parameters['src'] = 'hash';
		$parameters['lang'] = 'de';
		$tweet = $connection->get('search/tweets',$parameters);	*/
		...
		$output = '';
		//print_r($tweet); die;
		foreach($tweet as $t){	//$tweet->statuses
 ...
 **************************************************************************/
	...
	public function streamCSS($theme=false){		
		echo $this->getStreamCSS($theme);
	}
	
	public function streamJS(){
		echo $this->getStreamJS();
	}
	
	public function getStreamCSS($theme=false){
		if(empty($theme)) $theme = $this->get('theme');
		else $this->set('theme',$theme);
		$out =  "<link href='".$this->getBaseURL().NSS_CORE."core.css' type='text/css' rel='stylesheet' />\n";
		$out .= "<link href='".$this->getBaseURL().NSS_CONTENT.'themes/'.$theme."/style.css' type='text/css' rel='stylesheet' />\n";
		return $out;
	}
	
	public function getStreamJS(){
		$theme = $this->get('theme');
		$plugins = $this->getThemeMeta('Plugins:',$theme);
		$masonry = strpos($plugins,'masonry') ? 'true' : 'false';
		$fb_app_id = $this->get('fb_app_id');
		$intro_fadein = $this->get('intro_fadein');
		$auto_refresh = $this->get('cache_auto_refresh') ? 'true' : 'false';
		return "<script type='text/javascript' src='".$this->getBaseURL().NSS_CORE."jquery.neosmart.stream.js'></script>\n";
	}
 ...
 **************************************************************************/
	...
	public function autoLinkFacebookHashTags($string){
		$pattern = "/(#([0-9a-zA-ZäöüÄÖÜ][0-9a-zA-ZäöüÄÖÜß]+))/is";
		$string = preg_replace($pattern, " <a href='https://www.facebook.com/hashtag/$2' target='_blank' rel='nofollow'>$1</a>", $string);
		return $string;
	}
	
	public function autoLinkTwitterHashTags($string){
		$pattern = "/(#([0-9a-zA-ZäöüÄÖÜ][0-9a-zA-ZäöüÄÖÜß]+))/is";
		$string = preg_replace($pattern, " <a href='https://twitter.com/search?src=hash&q=%23$2' target='_blank' rel='nofollow'>$1</a>", $string);
		return $string;
	}
	...
	private function preventAutoplay($url){
		$url = str_replace('autoplay=true','autoplay=false',$url);
		$url = str_replace('autoplay=1','autoplay=0',$url);
		$url = str_replace('auto=true','auto=false',$url);
		$url = str_replace('auto=1','auto=0',$url);
		$url = str_replace('auto_play=true','auto_play=false',$url);
		$url = str_replace('auto_play=1','auto_play=0',$url);
		return $url;
	}
...
*****************************************************************************/
	...
	function testFilePermissions(){
		...
		$errors = array();
		$errors = $this->makeDir($errors,NSS_ABSPATH.NSS_CONFIG);
		$errors = $this->makeDir($errors,NSS_CONTENT_CACHE);
		$errors = $this->makeDir($errors,NSS_CONTENT_UPDATE);
	...
	function makeDir($errors,$path){
		if(!is_dir($path)){
			if(@mkdir($path, 0775)){
				if(!is_writable($path)){
					array_push($errors,
						array(	'<b>'.$path.'</b> is not writeable',
								'Set permission for <b>'.$path.'</b> to <b>0755</b> (chmod)'
						)
					);
				}
			}else{
				$chmod = substr(sprintf('%o', fileperms(NSS_ABSPATH)), -4);
				if($chmod=='0755'||$chmod=='0775'){
					array_push($errors,
						array(	'It is not possible to create new directories via PHP mkdir(). <b>'.NSS_ABSPATH.'</b> (CHMOD '.$chmod.') is not writeable.',
								'Try to set your webserver as owner of <b>'.NSS_ABSPATH.'</b><br>Please contact your server administrator to solve this issue. '
						)
					);
				}else{	
					array_push($errors,
						array(	'<b>'.NSS_ABSPATH.'</b> is not writeable',
								'Set permission for <b>neosmart-stream</b> to <b>0755</b> (chmod)'
						)
					);
				}
			}
 ...
 *****************************************************************************************/
	...
	function updateConfig(){
		...
		$data .= "\n$"."nss->set('date_default_timezone','".trim($_POST['date_default_timezone'])."');";
	...
	function reload($params='',$delay=0){
		if($delay===0){
			header('Location: '.$_SERVER['PHP_SELF'].$params);
		}else{
			include "../nss-admin/info.php";
		}
		die;
	}
